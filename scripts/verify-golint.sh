...
docker run --rm -v $(pwd)/cache:/cache -v $(pwd):/app --security-opt="label=disable" -e GOLANGCI_LINT_CACHE=/cache -w /app "golangci/golangci-lint:$VERSION" go work edit -json | jq -r '.Use[].DiskPath'  | xargs -I{} golangci-lint run {}/...
