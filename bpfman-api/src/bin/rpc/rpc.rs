...
use bpfman::{
    add_programs, attach_program, get_program, list_programs, pull_bytecode, remove_program, setup,
    types::{
...
impl Bpfman for BpfmanLoader {
    async fn load(&self, request: Request<LoadRequest>) -> Result<Response<LoadResponse>, Status> {
        ...
        let add_prog_result = add_programs(
            &config,
            &root_db,
            programs.into_iter().map(|p| p.unwrap()).collect(),
        )
    ...
    async fn attach(
        ...
        let link_id = attach_program(&config, &root_db, request.id, attach_info)
            .map_err(|e| Status::aborted(format!("{e}")))?;
