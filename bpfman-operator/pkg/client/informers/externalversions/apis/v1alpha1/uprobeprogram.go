...
// UprobeProgramInformer provides access to a shared informer and lister for
// UprobePrograms.
...
type uprobeProgramInformer struct {
	factory          internalinterfaces.SharedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
}

// NewUprobeProgramInformer constructs a new informer for UprobeProgram type.
...
func NewUprobeProgramInformer(client versioned.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredUprobeProgramInformer(client, resyncPeriod, indexers, nil)
}

// NewFilteredUprobeProgramInformer constructs a new informer for UprobeProgram type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
...
func (f *uprobeProgramInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredUprobeProgramInformer(client, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *uprobeProgramInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&apisv1alpha1.UprobeProgram{}, f.defaultInformer)
}

func (f *uprobeProgramInformer) Lister() v1alpha1.UprobeProgramLister {
	return v1alpha1.NewUprobeProgramLister(f.Informer().GetIndexer())
}
