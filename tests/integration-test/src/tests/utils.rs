...
macro_rules! add_program_inner {
    ($prog_inner_ty:expr, $prog_ty:expr, $config:ident, $root_db:ident, $name:ident, $location:ident, $globals:ident, $metadata:ident, $attach_info:ident) => {{
        let data = ProgramData::new($location, $name, $metadata, $globals, None).unwrap();
    ...
    ($prog_inner_ty:expr, $prog_ty:expr, $config:ident, $root_db:ident, $name:ident, $retprobe:ident, $location:ident, $globals:ident, $metadata:ident, $attach_info:ident) => {{
        let data = ProgramData::new($location, $name, $metadata, $globals, None).unwrap();
        let prog = $prog_inner_ty(data, $retprobe).unwrap();
        let res = add_programs($config, $root_db, vec![$prog_ty(prog)]).unwrap();
