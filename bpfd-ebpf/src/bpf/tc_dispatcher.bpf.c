...
volatile const struct tc_dispatcher_config TC_CONFIG = {};
...
{
    __u8 num_progs_enabled = TC_CONFIG.num_progs_enabled;
    int ret;
    ...
    if (num_progs_enabled < 1)
        goto out;
    ret = prog0(skb);
    bpf_printk("fuc: %d, ret: %d, chain: 0x%08X", 0, ret, TC_CONFIG.chain_call_actions[0]);
    if (!((1U << ret) & TC_CONFIG.chain_call_actions[0]))
        return ret;

    if (num_progs_enabled < 2)
        goto out;
    ret = prog1(skb);
    bpf_printk("fuc: %d, ret: %d, chain: 0x%08X", 1, ret, TC_CONFIG.chain_call_actions[1]);
    if (!((1U << ret) & TC_CONFIG.chain_call_actions[1]))
        return ret;

    if (num_progs_enabled < 3)
        goto out;
    ret = prog2(skb);
    bpf_printk("fuc: %d, ret: %d, chain: 0x%08X", 2, ret, TC_CONFIG.chain_call_actions[2]);
    if (!((1U << ret) & TC_CONFIG.chain_call_actions[2]))
        return ret;

    if (num_progs_enabled < 4)
        goto out;
    ret = prog3(skb);
    bpf_printk("fuc: %d, ret: %d, chain: 0x%08X", 3, ret, TC_CONFIG.chain_call_actions[3]);
    if (!((1U << ret) & TC_CONFIG.chain_call_actions[3]))
        return ret;

    if (num_progs_enabled < 5)
        goto out;
    ret = prog4(skb);
    if (!((1U << ret) & TC_CONFIG.chain_call_actions[4]))
        return ret;

    if (num_progs_enabled < 6)
        goto out;
    ret = prog5(skb);
    if (!((1U << ret) & TC_CONFIG.chain_call_actions[5]))
        return ret;

    if (num_progs_enabled < 7)
        goto out;
    ret = prog6(skb);
    if (!((1U << ret) & TC_CONFIG.chain_call_actions[6]))
        return ret;

    if (num_progs_enabled < 8)
        goto out;
    ret = prog7(skb);
    if (!((1U << ret) & TC_CONFIG.chain_call_actions[7]))
        return ret;

    if (num_progs_enabled < 9)
        goto out;
    ret = prog8(skb);
    if (!((1U << ret) & TC_CONFIG.chain_call_actions[8]))
        return ret;

    if (num_progs_enabled < 10)
        goto out;
    ret = prog9(skb);
    if (!((1U << ret) & TC_CONFIG.chain_call_actions[9]))
        return ret;
