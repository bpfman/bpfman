...
func GetPrimaryNodeInterface(ourNode *v1.Node) (string, error) {
	...
	for _, ipaddr := range ourNode.Status.Addresses {
		log.V(1).Info("Node IP  - ", "Type", ipaddr.Type, "Address", ipaddr.Address)
		if ipaddr.Type == v1.NodeInternalIP {
			for _, i := range ifaces {
				...
				for _, a := range addrs {
					switch v := a.(type) {
					case *net.IPAddr:
						log.V(1).Info("localAddresses", "name", i.Name, "index", i.Index, "addr", v, "mask", v.IP.DefaultMask())
						if ipaddr.Address == v.String() {
							log.Info("primary node interface set", "name", i.Name)
							return i.Name, nil
						}

					case *net.IPNet:
						log.V(1).Info(" localAddresses", "name", i.Name, "index", i.Index, "v", v, "addr", v.IP, "mask", v.Mask)
