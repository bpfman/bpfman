...
This design is intended to be built on top of the bpfman v0.5.4 release.
...
content of the CRDs (bpfman-operator/apis/v1alpha).
...
Then there is a \*Program struct, which includes in each \*Program’s Program struct and the base
...
## Adding Namespaced Scoped

To add Namespace scoped BPF Program and \*Program CRDs, Kubernetes requires new CRDs for each type.
Data will be the same, but just a duplicate type.

### struct

The set of CRD structures will need to be duplicated for each Namespaced scoped CRD (bpfman-operator/apis/v1alpha).
Note, data is the same, just a new object.
The `Namespace` field is embedded the `metav1.ObjectMeta` structure.
...
To allow the common code to act on both Cluster or Namespaced objects, two new interfaces will be introduce.
First is `BpfProg`.
...
Around the same time this feature is being developed, another featurer is being developed
which will break the loading and attaching of eBPF programs in bpfman in to two steps.
...
carry the identifier and just append to the `v1alpha1` api.
Then rework all the CRDs for the load and attach feature, using the "Cluster" identifier where required,
and create a `v1alpha2` for those changes.
