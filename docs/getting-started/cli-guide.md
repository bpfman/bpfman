...
* **Attach Program**: The next step is to attach a loaded eBPF Program to a hook point
  ...
  Once attached, the eBPF Program will be called if its hook point is triggered
...
They need to be loaded in one command so that each of those eBPF programs can share any global
