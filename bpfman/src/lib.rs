...
/// into the system:
///
/// 1. Initialises the configuration and database.
/// 2. Sets up the map pin path if the program uses another program's maps.
/// 3. Depending on the program type, it either sets up multi-attach
///    or single-attach:
///    - XDP and TC programs are multi-attached.
///    - Tracepoint, Kprobe, Uprobe, Fentry, and Fexit programs are
///      single-attached.
/// 4. If the program is successfully added, it updates the program ID
///    and map ownership and allows access to all maps by the bpfman
///    group members.
/// 5. In case of failure, it cleans up any created directories or
///    loaded programs to maintain system integrity.
///
/// # Arguments
///
...
///
///     // Initialise metadata for the program. Metadata contains custom
///     // configurations or properties related to the eBPF program, such
///     // as threshold values, sampling rates, specific flags (e.g.,
///     // enabling debug mode), and other parameters. These values can be
///     // used by the userspace program.
///     let mut metadata = HashMap::new();
///     metadata.insert("cpu_usage_threshold".to_string(), "80".to_string());
///     metadata.insert("sampling_rate".to_string(), "1000".to_string());
///     metadata.insert("enable_debug".to_string(), "true".to_string());
///
///     // Initialise global data for the program. Global data holds
///     // variables that can be shared across multiple eBPF programs,
///     // such as counters or aggregated statistics. The keys in
///     // global_data must already exist in the eBPF program, otherwise,
///     // an error will be encountered. For example:
///     // global_data.insert("global_counter".to_string(), vec![0; 8]);
///     let global_data = HashMap::new();
///
///     // Summary:
///     //
///     // Metadata is used by the userspace program and does not need to
///     // exist in the eBPF program. It contains configurations and
///     // properties that guide how the userspace program interacts with
///     // the eBPF program.
///     //
///     // Global data is shared between the userspace program and the
///     // eBPF program. The keys must be defined in the eBPF program's
///     // BPF maps because the eBPF program directly interacts with these
///     // data structures.
///
...
/// Unloads an eBPF program specified by its ID.
///
...
/// # Logging
///
/// The logging level is governed by the `RUST_LOG` environment variable:
/// * To see `debug!` messages, set `RUST_LOG` to `debug` or lower
///   (`trace`).
/// * `warn!` messages are logged by default in both debug and release
///   builds.
///
/// * `debug!("BpfManager::list_programs()")` - Logged at the start of
///   the function to indicate that the `list_programs` function has
///   been called. This is only logged in debug builds or if the log
///   level is set to `debug` or lower.
///
/// * `warn!("Unable to set kernel info for prog {prog_id}, error: {e}")` -
///   Logged if there is an error while setting kernel information for
///   a program. This is logged in both debug and release builds and
///   helps in identifying issues with retrieving kernel information
///   without stopping the listing of other programs.
///
/// # Example
///
/// ```rust,no_run
