...
fn attached_programs(did: &DispatcherId, root_db: &Db) -> usize {
...
async fn add_multi_attach_program(
    ...
    let next_available_id = attached_programs(&did, root_db);
...
async fn remove_multi_attach_program(
    ...
    let next_available_id = attached_programs(&did, root_db) - 1;
    debug!("next_available_id = {next_available_id}");
