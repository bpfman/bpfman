...
impl BpfManager {
    ...
    fn get_dispatcher(&self, id: &DispatcherId) -> Option<Dispatcher> {
        ...
        ROOT_DB
            .get()
            .expect("ROOT_DB is not initialized")
            .tree_names()
            .into_iter()
    ...
    fn get(&self, id: &u32) -> Option<Program> {
        let prog_tree: sled::IVec = (PROGRAM_PREFIX.to_string() + &id.to_string())
            .as_bytes()
            .into();
        if root_db_init(&self.config).tree_names().contains(&prog_tree) {
            let tree = ROOT_DB
                .get()
                .expect("unable to open database")
                .open_tree(prog_tree)
    ...
    pub(crate) async fn rebuild_state(&mut self) -> Result<(), anyhow::Error> {
        ...
        for tree_name in root_db_init(&self.config).tree_names() {
    ...
    ROOT_DB
        ...
        .map(|n| {
            ROOT_DB
                ...
                .expect("unable to open database")
                .open_tree(n)
                .expect("unable to open map tree")
        })
}
